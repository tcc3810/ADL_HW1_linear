    ...
    with torch.no_grad():
        ...
        for i , batch in enumerate(tqdm(test_loader)):
            ...
            for j in range(len(preds)):
                ...
                # classification
                x_preds.append(data_csv.split())
            
            # classification
            labels = batch["tags"].to(device)
            for j in range(len(labels)):
                label_data = ""
                tmp = labels.view(-1).cpu().numpy()
                for k in range(batch["len"][j]):
                    label_data += dataset.idx2label(tmp[args.max_len * j + k]) + " "
                
                label_data = label_data[:-1]
                x_labels.append(label_data.split())
            
        print(classification_report(x_labels , x_preds , mode='strict' , scheme=IOB2))
